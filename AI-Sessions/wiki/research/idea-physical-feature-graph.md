...
## 2026-07-18 정제: "계산된 관계" 원칙의 credit 축 확장

위 표의 "남들이 학습하는 coupling을 계산해서 주입한다"는 구분은 representation에만 머물지 않고 **credit assignment 축에서 그대로 반복**된다. per-agent advantage 선행연구도 influence를 학습으로 얻는다 — GPAE(AAMAS 2026)는 counterfactual critic으로 **근사**하고, dependence-graph credit(arXiv 2601.21523)은 reverse-model MI로 **추론**한다. 이 연구는 그 자리를 물리로 대체한다: centroidal 축에는 CMM($A_G$), kinematic tracking 축에는 kinematic reachability. 즉 하나의 원칙("influence는 학습 말고 계산")이 representation(이 note)과 credit([[AI-Sessions/wiki/research/idea-centroidal-momentum-allocation-credit|allocation credit]]) 두 층에서, 그리고 centroidal·mimic 두 도메인에서 반복 검증되는 구조다.

- centroidal 축 정본: [[AI-Sessions/wiki/research/idea-gpae-centroidal-advantage|idea-gpae-centroidal-advantage]] (GPAE = S2 배관·learned counterfactual 대조군)
- mimic IL 축 정본: [[AI-Sessions/wiki/research/idea-kinematic-dependence-credit|idea-kinematic-dependence-credit]] (kinematic reachability를 oracle dependence graph로; 3-way 비교표 보유)
...
- 상위 인사이트 (2026-07-18 재배치): [[AI-Sessions/wiki/research/idea-model-based-critic|idea-model-based-critic]] — 이 note는 그 표현 축(물리를 actor 관측/attention에 주입)
