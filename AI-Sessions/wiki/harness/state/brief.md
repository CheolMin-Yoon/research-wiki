...
- `/home/frlab/mj_rl` current branch is `master`; 2026-07-04 작업은 BoT mapping/readability, `waist_momentum` leg+waist task, flat prefixed CasADi-on-GPU kernels를 반영했다.
- 자세한 현재 계약은 `mj-rl.md`, CasADi backend 계약은 `casadi-on-gpu-code.md`, 오래된 reflect 상세는 archive history를 본다.
- `G1-WaistMomentum-Locomotion` zero-agent smoke는 통과했지만, native viewer에서 사람이 momentum arrow scale/readability를 확인하는 작업은 남아 있다.
