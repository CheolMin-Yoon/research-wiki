...
- `/home/frlab/mj_rl`이 branch `refactor/mj-rl-v2`로 전면 재작성됐다(checked commit `c890973`, 2026-07-11). 이전 `modules/primitives`·`tasks.bot_velocity`·`centroidal`·`eicp`·`waist_momentum` 구조는 사라졌고, 지금은 `source/{assets,rl,tasks,utils}` + `tasks.mlp_ctde` 단일 task뿐이다. v1 상세는 `archived-mj-rl-v1-2026-07-11.md`를 본다 — **이전 세션의 mj_rl 기억(모듈 3분할, BoT task 등)은 이제 stale이다.**
...
- 자세한 현재 계약(검증 범위 명시)은 `mj-rl.md`, 일반화 원칙은 `mjlab-patterns.md`("one token per domain concept"), CasADi backend 계약은 `casadi-on-gpu-code.md`를 본다. `mj-rl.md`는 이번 세션이 직접 연 파일만 채워졌고 `rl/storage.py`·`rl/routing.py`·`tasks/mlp_ctde/mdp/*`·`utils/`·`scripts/`는 다음 세션 재감사 대상이다.
