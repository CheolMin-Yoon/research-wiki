...
- CMM 커널 DOF/관성 검증(2026-07-01): CasADi/CMM 커널은 **23-DOF locked spec** 기준(legs 12 + waist_yaw 1 + arms 10)이고, lock은 질량 drop이 아니라 부모 바디로 **관성 lumping**(총질량 33.34kg 보존, 중립자세 고정). action surface DOF와 별개. 정본 = mj-rl.md "2026-07-01 Reflect".
- BoT master cleanup + graph/token visualization review(2026-07-01): `source/assets/cuda`와 `scripts/casadi_on_gpu`는 삭제 대상이 아니라 centroidal optional 자산으로 보존. 정본 = `AI-Sessions/wiki/research/sources/mj-rl.md` "2026-07-01 Reflect: BoT master cleanup + graph/token visualization review"; 삭제 오판 교훈 = `AI-Sessions/wiki/harness/errors/mjlab-errors.md`.
- mj_rl housekeeping(2026-07-03): `/home/frlab/mj_rl` HEAD `7173d3e`. `NOTES.md` 압축, native viewer Ctrl-C 종료 보정, K1 `ElectricActuator` wrapper 정리, JAX probe deps 기본 env 제거가 반영됐다. 정본 = `AI-Sessions/wiki/research/sources/mj-rl.md` "2026-07-03 Reflect: NOTES compression, play shutdown, K1 actuator wrapping".
