...
`/home/frlab/mj_rl` branch `refactor/mj-rl-v2`의 commit `f733adf`까지 `G1-FALCON` 29-DOF parity와 향후 graph task용 순수 force tensor math 공통화를 구현했다. 구현 정본은 `mj-rl.md`를 본다.
...
2. 16-env 단기 학습으로 curriculum progression을 확인한다.

## Current Facts

- `/home/frlab/mj_rl` checked commit은 `f733adf`(`refactor/mj-rl-v2`)이다.
...
- 검증: 전체 64 tests OK, CPU 2-env reset-wrench rollout, GPU 4-env MAPPO 1 iteration OK.
