...
- paper/source는 hub처럼 확장하지 않는 leaf로 둔다. 단, 관계 정본인 `## Links` wikilink는 허용한다.
...
|---|---|
| paper | related concept note + research.md |
...
- **leaf (paper·source·concept·idea·experiment)** — 실제 지식 정본. 관계는 `## Links` wikilink로 표현(Research Rules 참조).
