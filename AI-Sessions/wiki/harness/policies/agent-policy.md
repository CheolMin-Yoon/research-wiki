...
4. **detail**: 라우터가 가리키는 `{domain}-*.md` 또는 세 공개 명령의 `prompts/<command>.md`만.
...
사용자에게 노출하는 명령은 세 개다. 상세 규칙은 `prompts/<command>.md`에 있다.
...
다음은 공개 명령이 아니라 세 명령과 일반 작업 뒤에서 자동으로 적용하는 수명주기다.

1. **Resume context** — 사용자가 이어서 하기, 계속, 현재 상태를 요청하면 `architecture → brief → handoff` 순서로 복원하고 관련 노트만 읽는다. 최근 변화가 꼭 필요할 때만 `log.md`를 읽는다.
2. **Durable capture** — ingest/reflect는 재사용 지식, 필수 handoff, 추적 가능한 결정 근거, 반복 실패 위험, 공유 규칙 중 하나를 만족할 때만 쓴다. 단순 진행 상황과 코드에서 재구성 가능한 설명은 저장하지 않는다.
3. **Post-write validation** — 구조·schema·link·Bases·graph·prompt write 뒤 doctor를 실행한다. schema/script 변경은 unit tests, stable graph 변경은 Louvain check까지 실행하고 실패를 해결하거나 보고한다.
4. **Archive maintenance** — 정리·폐기 요청이 오면 `archive-policy.md`를 on demand로 읽고 status/archive/Git history 정책을 적용한다. 별도 명령 파일은 두지 않는다.
