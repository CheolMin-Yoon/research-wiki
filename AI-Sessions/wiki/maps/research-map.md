...
- [[idea-model-based-critic]]
- [[AI-Sessions/wiki/maps/resources|resources]]
- [[AI-Sessions/wiki/research/experiments/experiments|experiments]]

## Summary

research 섬의 root(tier/top)다. 다른 섬(harness/docs)과 edge로 연결되지 않는 독립 트리다. graph-visible 흐름은 `research-map -> 메인 인사이트(idea-model-based-critic) -> 하위 idea -> category -> paper`이며, resources/experiments는 research root의 별도 상위 가지다. 기존 idea들(physical-feature-graph, centroidal-momentum-allocation-credit과 그 파생)은 메인 인사이트의 하위 축으로 연결된다(2026-07-18 재배치).
