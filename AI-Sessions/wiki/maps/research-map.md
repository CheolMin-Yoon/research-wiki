...
상위 tier는 idea 노트, resources(소스 허브), experiments다. idea는 근거 concept(중위: `transformer`, `ppo`, `lipm`, `centroidal`)로, concept는 근거 paper(하위)로 내려간다. resources는 source/repo/code 분석 노트(하위)로 뻗는다. paper/source는 graph leaf이며 outgoing wikilink를 만들지 않는다.
