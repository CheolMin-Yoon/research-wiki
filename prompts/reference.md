---
tags: [tier/low]
---

# Reference Prompt

```text
지금 작업하려는 주제에 대해 옵시디언을 참조(reference)해줘.

먼저 architecture.md, log.md, AI-Sessions/wiki/harness/state/brief.md를 확인해줘.

그다음 AI-Sessions/wiki/maps, AI-Sessions/wiki/research, AI-Sessions/wiki/harness 안에서 관련 노트만 찾아서:

1. 핵심 맥락과 이미 정리된 인사이트
2. 관련 레퍼런스 구현에서 가져올 수 있는 패턴
3. 과거 결정사항과 반복하면 안 되는 함정
4. 지금 이어서 할 다음 액션

을 요약하고, 현재 구현/연구에 대한 조언을 제시해줘.

reference는 읽기 전용이므로 새 문서를 만들지 마. 저장이 필요하면 별도 save 또는 reflect로 전환해줘.
```
