...
raw 원본은 읽기 전용으로 취급하고 수정·삭제하지 마. 새 wiki 문서를 만들기 전에 architecture.md와 관련 wiki 노트를 먼저 확인해서 중복을 피하고, 같은 주제가 있으면 새 문서보다 기존 노트 갱신을 우선해줘. 상충하는 내용은 덮어쓰지 말고 "상충:"으로 표시해줘.
...
새 graph 노트(paper/source/idea 등)는 frontmatter에 `tier/*` 태그(보통 paper/source는 low, idea는 upper)를 부여하고 해당 섬 hub에 등록해줘(obsidian-policy 참조). 작업이 끝나면 architecture.md와 필요한 map/state를 갱신해줘. log.md는 큰 ingest나 운영 규칙 변화일 때만 한 줄로 남겨줘.
